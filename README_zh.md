...
此项目完全由 AI 开发，经过数十轮优化，以下是创作历程：

- 开始的目标是想做一个自动发布小说章节到番茄小说的脚本程序，中间肯定要用到大模型。
- 向Gemini和Grok普通聊天询问方案，随后选定browser-use这个开源组件。中间也试过其他AI浏览器、插件之类的，不满意。
- 确定方案后使用VSCode + KiloCode代理，大模型先后使用GLM 4.7, Minimax 2.5, GLM 5, Step 3.5 Flash, Qwen3.5 397B/122B等。
- 程序始终不稳定，各种小问题，最后切换到Gemini 3.1 Pro，很快修复稳定下来，并不断加入新功能，基本都是一次性搞定。
- 本人也算是资深码农（1995年开始编程），但没用过Python，所以前后没有调试过一行代码，甚至没怎么看过代码。
- 我大约使用了一整天的时间完成了这个项目。
